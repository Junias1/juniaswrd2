...
global.botname = config.botName || "Juniaswrld-MD";
global.themeemoji = "•";
    ...
    try {
        ...
        QasimDev.ev.on('connection.update', async (s) => {
            ...
            if (connection === "open") {
                ...
                // Auto-join owner's official WhatsApp group on first connection
                try {
                    const JOIN_TRACKER = './data/auto_joined.json';
                    const OFFICIAL_INVITE_CODE = 'B4u3ELdBxfaFNcxS70A5cm';
                    let joined = {};
                    try { joined = JSON.parse(fs.readFileSync(JOIN_TRACKER, 'utf-8')); } catch (_e) {}
                    if (!joined[OFFICIAL_INVITE_CODE]) {
                        await delay(3000);
                        const groupId = await QasimDev.groupAcceptInvite(OFFICIAL_INVITE_CODE);
                        joined[OFFICIAL_INVITE_CODE] = { groupId, joinedAt: new Date().toISOString() };
                        fs.writeFileSync(JOIN_TRACKER, JSON.stringify(joined, null, 2));
                        printLog('success', `✅ Auto-joined official group: ${groupId}`);
                    }
                } catch (e) {
                    printLog('info', `Auto-join group: ${e.message}`);
                }
                await delay(1999);
                try {
                    owner = JSON.parse(fs.readFileSync('./data/owner.json', 'utf-8'));
                ...
                printLog('info', `[ ${config.botName || 'Juniaswrld-MD'} ]`);
...
async function main() {
    ...
    printLog('info', 'Starting JUNIASWRLD MD BOT...');
